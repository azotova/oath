...
var resolveChildren = function (promiseObj, data) {
  var newestPromise = promiseObj.waitFun(data);
  var addChilren = function (promiseNew, promiseOld) {
    if (promiseOld.waitProm.length === 0) {
      return;
    } else {
      for (var i=0; i < promiseOld.waitProm.length; i++) {
        return addChildren(promiseNew.then(promiseOld.waitProm[i].waitFun),promiseOld.waitProm[i]);
      }
    }
  }
  return addChilren(newestPromise, promiseObj);
