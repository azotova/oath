...
var Promise = function (value, status, context) {
  ...
  if (!context || resolved.indexOf(context) !== -1) {
    ...
    promiseObj.waitFun = value;
    context.waitProm.push(promiseObj);
  } 
...
Deferred.prototype.resolve = function (data, sec) {
  ...
  for (var j = 0; j < this.promise.waitProm.length; j++) {
    waiting.promises.push(this.promise.waitProm[i]); //add to the global object;
    this.promise.waitProm[i].waitFun(data);
      for (var k = 0; k < this.promise.waitProm[i].waitProm.length; k++) {
